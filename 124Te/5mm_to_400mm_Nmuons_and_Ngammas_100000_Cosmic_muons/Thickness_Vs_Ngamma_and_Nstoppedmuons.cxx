...
double Nmuons[Size];
double NGammas[Size];
double NGammas_Per_Decimeter[Size];
...
//-----------------The following code is used to get the number of gamma per suqare decimeter.------------------------------
     ...
     for (int i = 0; i < Size; i++)
     {
        //NGammas_Per_Decimeter[i] = NGammas[i]*1.0/(3.1415*4*2+2*3.1415*2*px[i]*0.01);
        NGammas_Per_Decimeter[i] = NGammas[i]*1.0/(3.1415*0.04*2+2*3.1415*0.2*px[i]*0.001);
        cout << "NGammas_Per_Decimeter["<<i<<"]: "<<NGammas_Per_Decimeter[i] << " \n";
     }		
...
//-------------------------The following code is used to draw Graph------------------
   ...
   auto gr2 = new TGraph( Size, px, NGammas_Per_Decimeter);
